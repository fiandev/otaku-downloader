...
class Downloader {
  static async zippy(url) {
    if (new Date().getTime() > new Date("31, march 2023 23:59:59").getTime()) {
      info(`since ${ colors.cyan("31 march 2023 at 23:59:59") }.\nthe ${ colors.yellow("zippyshare") } file hosting has been ${ colors.red("shutdown permanently") }.`);
      process.exit();
    }
    try {
      ...
      let episodeList = result.episode_list.map((episode, i) => episode.title);
